  ...
  return (
    ...
    >
      <div className="flex h-full flex-col">
        {/* Header */}
        <div className="shrink-0 border-b-2 border-black/10 bg-white p-4">
          {/* Breadcrumb */}
          <div className="mb-3 flex items-center gap-1 text-xs text-black/60">
            <Link href="/roadmap" className="hover:text-black">
              Roadmap
            </Link>
            <ChevronRight className="h-3 w-3" />
            <span className="font-medium text-black">Module {week.week}</span>
            <ChevronRight className="h-3 w-3" />
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 flex-1">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-md border-2 border-black bg-white px-2 py-0.5 text-xs font-bold uppercase">
                  {task.difficulty}
                </span>
              </div>
              <h2 className="font-display text-xl font-bold">{task.title}</h2>
              <p className="mt-1 flex items-center gap-3 text-sm text-black/60">
            ...
            <button
              onClick={onBack}
              className="shrink-0 rounded-md border-2 border-black bg-white p-2 transition-colors hover:bg-gray-100"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="border-b-2 border-black/10 lg:border-b-0 lg:border-r-2 lg:w-80 lg:shrink-0">
              <div className="p-4">
                ...
                <div className="space-y-2">
                  {(task.resources ?? []).filter((r) => r.resourceType === 'youtube').length > 0 && (
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-black/60">Videos</p>
                      {(task.resources ?? [])
                        ...
                        .map((resource) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setTimeout(() => {
                                const el = document.getElementById(`resource-${resource.id}`)
                          ...
                          >
                            <div className="h-8 w-12 shrink-0 overflow-hidden rounded border border-black/20">
                  ...
                  {(task.resources ?? []).filter((r) => r.resourceType === 'docs' || r.resourceType === 'article').length > 0 && (
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-black/60">Documentation</p>
                      {(task.resources ?? [])
                        ...
                        .map((resource) => (
                          ...
                          >
                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-blue text-white">
                ...
                <div className="mt-4 space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    {task.quizRequired !== false && (
                      <div>
                        {canOpenQuiz ? (
                          <Link href={`/roadmap/tasks/${task.id}/quiz`}>
                            <BrutalButton
                        ...
                        {quizLockReason && !task.quizPassed && (
                          <p className="mt-1 text-[10px] text-black/60">{quizLockReason}</p>
                        )}
                      </div>
                    )}
                    ...
                    {hasMiniProject && (
                      <div>
                        {canOpenProject ? (
                          <Link href={`/roadmap/tasks/${task.id}/project`}>
                            <BrutalButton
                        ...
                        {projectLockReason && !task.projectPassed && (
                          <p className="mt-1 text-[10px] text-black/60">{projectLockReason}</p>
                        )}
                      </div>
                    )}
                  </div>
